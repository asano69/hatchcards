	...
	m.Register(func(app core.App) error {
		jsonData := `[
			...
			},
			{
				...
				"type": "view",
				"updateRule": null,
				"viewQuery": "\nSELECT\n(ROW_NUMBER() OVER()) AS id,\n  f.deck_name,\n  CAST((julianday(date(c.due_date)) - julianday(date('now'))) / 7 AS INTEGER) AS week_offset,\n  COUNT(*) AS count\nFROM cards c\nJOIN files f ON f.relation = c.id\nWHERE c.due_date IS NOT NULL AND c.due_date != ''\n  AND date(c.due_date) >= date('now')\n  AND date(c.due_date) < date('now', '+84 days')\nGROUP BY f.deck_name, week_offset\nORDER BY f.deck_name, week_offset;",
				"viewRule": ""
			},
			{
				"createRule": null,
				...
				"type": "view",
				"updateRule": null,
				"viewQuery": "SELECT\n  (ROW_NUMBER() OVER()) AS id,\n  deck_name,\n  month_offset,\n  COUNT(*) AS count\nFROM (\n  SELECT\n    f.deck_name,\n    (CAST(strftime('%Y', c.due_date) AS INTEGER) - CAST(strftime('%Y', 'now') AS INTEGER)) * 12\n      + (CAST(strftime('%m', c.due_date) AS INTEGER) - CAST(strftime('%m', 'now') AS INTEGER))\n      AS month_offset\n  FROM cards c\n  JOIN files f ON f.relation = c.id\n  WHERE c.due_date IS NOT NULL AND c.due_date != ''\n    AND date(c.due_date) >= date('now')\n)\nWHERE month_offset >= 0 AND month_offset < 12\nGROUP BY deck_name, month_offset\nORDER BY deck_name, month_offset;",
				"viewRule": ""
			},
			{
				"createRule": null,
				...
				"type": "view",
				"updateRule": null,
				"viewQuery": "SELECT\n  (ROW_NUMBER() OVER()) AS id,\n  f.deck_name,\n  -- Cards never reviewed yet\n  SUM(CASE WHEN c.due_date = '' THEN 1 ELSE 0 END) AS new_count,\n  -- Cards due today or overdue (reviewing always pushes due_date to\n  -- tomorrow or later, so today's reviews are naturally excluded)\n  SUM(CASE WHEN c.due_date != '' AND date(c.due_date) <= date('now')\n           THEN 1 ELSE 0 END) AS due_count,\n  -- Cards reviewed at least once today\n  SUM(CASE WHEN c.last_reviewed_at != '' AND date(c.last_reviewed_at) = date('now')\n           THEN 1 ELSE 0 END) AS reviewed_today_count\nFROM cards c\nJOIN files f ON f.relation = c.id\nGROUP BY f.deck_name\nORDER BY f.deck_name;",
				"viewRule": null
			},
			{
				"createRule": null,
